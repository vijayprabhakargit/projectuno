...
io.on('connection', (socket) => {
  ...
  socket.on('room:start_game', () => {
    if (currentRoomId) {
      ...
      if (result.success && result.gameState) {
        ...
        io.to(currentRoomId).emit('game:started', result.gameState);
  ...
  socket.on('game:play_card', (cardId: string, chosenColor?: CardColor) => {
    ...
    const { UnoGame } = require('./game/game');
    const game = new UnoGame(room.players.map(p => ({ id: p.id, name: p.name })));
    ...
    if (result.success) {
      ...
      if (room.gameState.winner) {
        io.to(currentRoomId).emit('game:game_over', room.gameState.winner, room.gameState);
      }
    } else {
